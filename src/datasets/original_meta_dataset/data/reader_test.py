...
DATASET_SPEC = DatasetSpecification(
    name=None,
    classes_per_split={Split.TRAIN: 15, Split.VALID: 5, Split.TEST: 10},
...
def bind_gin_parameters():
    ...
    gin.bind_parameter(
        "EpisodeDescriptionConfig.max_ways_upper_bound", MAX_WAYS_UPPER_BOUND
    )
    gin.bind_parameter("EpisodeDescriptionConfig.max_num_query", MAX_NUM_QUERY)
    gin.bind_parameter(
        "EpisodeDescriptionConfig.max_support_set_size", MAX_SUPPORT_SET_SIZE
    )
    gin.bind_parameter(
    ...
    gin.bind_parameter("EpisodeDescriptionConfig.ignore_hierarchy_probability", 0.0)
    gin.bind_parameter("EpisodeDescriptionConfig.simclr_episode_fraction", 0.0)
...
class DatasetIDGenTest(tf.test.TestCase):
    ...
    def check_expected_structure(self, sampler):
        ...
        tf_generator = tf.data.Dataset.from_generator(
            generator, tf.int64, tf.TensorShape([None, 2])
        ).map(reader.decompress_episode_representation)
...
def construct_fake_datasets(
    class_ids, examples_per_class, repeat=True, shuffle=True, shuffle_seed=None
):
...
class FakeEpisodeReader(reader.EpisodeReader):
    ...
    def construct_class_datasets(
        ...
        return construct_fake_datasets(
            class_ids, examples_per_class, repeat, shuffle, shuffle_seed
        )
...
class EpisodeReaderTest(tf.test.TestCase):
    ...
    def generate_episodes(self, sampler, num_episodes, shuffle=True, shuffle_seed=None):
        ...
        episode_reader = FakeEpisodeReader(
            dataset_spec,
            split,
            shuffle_buffer_size,
    ...
    def check_end_padding(self, examples_chunk, targets_chunk):
        ...
        self.assertTrue(
            all(
                reader.PLACEHOLDER_CLASS_ID == target
    ...
    def check_description_vs_target_chunks(
        ...
        self.assertTrue(
            all(target_support_chunk[support_cursor:] == reader.PLACEHOLDER_CLASS_ID)
        )
        self.assertTrue(
    ...
    def test_flush_logic(self):
        ...
        dataset_spec = DatasetSpecification(
            name=None,
            classes_per_split={Split.TRAIN: num_classes, Split.VALID: 0, Split.TEST: 0},
