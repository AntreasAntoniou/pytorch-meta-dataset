...
def validate_spanning_leaves(spanning_leaves, subset_synsets, test_instance):
    ...
    for s, leaves in spanning_leaves.items():
        ...
        for c in s.children:
            if not c.children:
                num_leaf_children += 1
            test_instance.assertGreaterEqual(
                num_spanning_leaves, len(spanning_leaves[c])
            )
...
def test_lowest_common_ancestor_(lca, height, leaf_a, leaf_b, test_instance, root=None):
...
class SplitCreationTest(tf.test.TestCase):
    ...
    def validate_roots(self, valid_test_roots, spanning_leaves):
        ...
        self.assertGreaterEqual(
            num_valid_leaves, DESIRED_TOY_NUM_VALID_CLASSES - TOY_MARGIN
        )
        self.assertLessEqual(
            num_valid_leaves, DESIRED_TOY_NUM_VALID_CLASSES + TOY_MARGIN
        )
        self.assertGreaterEqual(
            num_test_leaves, DESIRED_TOY_NUM_TEST_CLASSES - TOY_MARGIN
        )
        self.assertLessEqual(num_test_leaves, DESIRED_TOY_NUM_TEST_CLASSES + TOY_MARGIN)
...
class ImagenetSpecificationTest(tf.test.TestCase):
    def validate_num_span_images(self, span_leaves, num_span_images):
        ...
        for node, leaves in span_leaves.items():
            self.assertEqual(
                num_span_images[node], sum([num_span_images[l] for l in leaves])
            )
    ...
    def test_imagenet_specification(self):
        ...
        valid_subgraph, test_subgraph = (
            splits[learning_spec.Split.VALID],
            splits[learning_spec.Split.TEST],
        )
