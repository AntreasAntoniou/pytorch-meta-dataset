...
def get_total_images_per_class(data_spec, class_id=None, pool=None):
    ...
    if pool is None:
        if isinstance(num_images, collections.Mapping):
            raise ValueError(
                "DatasetSpecification {} has example-level splits, so "
...
class BenchmarkSpecification(
    collections.namedtuple(
        ...
        "name, image_shape, dataset_spec_list,"
    ...
    def __new__(
        ...
        if len(has_dag_ontology) != len(dataset_spec_list):
            raise ValueError(
                "The length of has_dag_ontology must be the number of " "datasets."
            )
        ...
        for i, has_dag in enumerate(has_dag_ontology):
            if has_dag and not isinstance(
                dataset_spec_list[i], HierarchicalDatasetSpecification
            ):
                raise ValueError(
                    "Dataset {} has dag ontology, but does not have a "
                    "hierarchical dataset specification.".format(i)
                )
            if not has_dag and isinstance(
                dataset_spec_list[i], HierarchicalDatasetSpecification
            ):
                raise ValueError(
        ...
        for i, is_bilevel in enumerate(has_bilevel_ontology):
            if is_bilevel and not isinstance(
                dataset_spec_list[i], BiLevelDatasetSpecification
            ):
                raise ValueError(
                    "Dataset {} has bilevel ontology, but does not have a "
                    "bilevel dataset specification.".format(i)
                )
            if not is_bilevel and isinstance(
                dataset_spec_list[i], BiLevelDatasetSpecification
            ):
                raise ValueError(
        ...
        for dataset_spec, dataset_splits in zip(
            dataset_spec_list, splits_to_contribute
        ):
            ...
            invalid_train_split = (
                ...
                and not classes_per_split[learning_spec.Split.TRAIN]
            )
            invalid_valid_split = (
                "valid" in dataset_splits
                and not classes_per_split[learning_spec.Split.VALID]
            )
            invalid_test_split = (
                "test" in dataset_splits
                and not classes_per_split[learning_spec.Split.TEST]
            )
...
class BiLevelDatasetSpecification(
    ...
    def _get_split_offset(self, split):
        ...
        if split == learning_spec.Split.TRAIN:
            ...
            previous_superclasses = range(
                0,
                self.superclasses_per_split[learning_spec.Split.TRAIN]
    ...
    def get_class_ids_from_superclass_subclass_inds(
        self, split, superclass_id, class_inds
    ):
...
def as_dataset_spec(dct):
    ...
    def _key_to_split(dct):
        ...
        return {learning_spec.Split[key]: value for key, value in six.iteritems(dct)}
...
def load_dataset_spec(dataset_records_path, convert_from_pkl=False):
    ...
    if tf.io.gfile.exists(json_path):
        ...
        raise RuntimeError(
            "No dataset_spec file found in directory %s" % dataset_records_path
        )
