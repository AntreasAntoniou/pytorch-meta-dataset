...
def log_graph_stats(
    ...
    logging.info(
        "Number of leaf images: min %d, max %d, median %f",
        min(num_leaf_images),
        max(num_leaf_images),
...
def log_stats_finegrainedness(
    ...
    name_message = (
        " of the {} graph".format(graph_name) if graph_name is not None else ""
    )
