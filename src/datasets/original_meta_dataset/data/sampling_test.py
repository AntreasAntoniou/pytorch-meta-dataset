...
class ComputeNumQueryTest(tf.test.TestCase):
    ...
    def test_max_num_query_respected(self):
        ...
        num_query = sampling.compute_num_query(
            images_per_class, max_num_query=test_utils.MAX_NUM_QUERY, num_support=None
        )
...
class EpisodeDescrSamplerErrorTest(parameterized.TestCase, tf.test.TestCase):
    ...
    @parameterized.named_parameters(
        ("num_ways_none", None, 5, 10, {}),
        ("num_ways_none2", None, 5, 10, {"min_ways": 3}),
        ("num_support_none", 5, None, 10, {}),
        ("num_support_none2", 5, None, 10, {"max_support_set_size": 3}),
    ...
    @parameterized.named_parameters(
        ("num_ways_none", None, 5, 10, {"min_ways": 3, "max_ways_upper_bound": 5}),
        (
            "num_support_none",
            5,
            None,
        ...
        ("num_query_none", 5, 5, None, {"max_num_query": 3}),
    )
...
class EpisodeDescrSamplerTest(tf.test.TestCase):
    ...
    def test_max_examples(self):
        ...
        for _ in range(10):
            ...
            self.assertTrue(
                all(
                    s + q
                    <= self.dataset_spec.get_total_images_per_class(class_set[cid])
    ...
    def test_non_deterministic(self):
        ...
        for _ in range(10):
            ...
            raise AssertionError(
                "Different EpisodeDescriptionSamplers generate "
    ...
    def assert_expected_chunk_sizes(
        self, expected_support_chunk_size, expected_query_chunk_size
    ):
...
class FixedQueryEpisodeDescrSamplerTest(EpisodeDescrSamplerTest):
    ...
    def test_query_too_big(self):
        ...
        sampler = sampling.EpisodeDescriptionSampler(
            self.dataset_spec, self.split, config.EpisodeDescriptionConfig(num_query=10)
        )
        with self.assertRaises(ValueError):
...
class FixedWaysEpisodeDescrSamplerTest(EpisodeDescrSamplerTest):
    ...
    def test_correct_chunk_sizes(self):
        self.assert_expected_chunk_sizes(
            test_utils.MAX_SUPPORT_SET_SIZE, self.num_ways * test_utils.MAX_NUM_QUERY
        )


class FixedEpisodeDescrSamplerTest(
    FixedShotsEpisodeDescrSamplerTest, FixedWaysEpisodeDescrSamplerTest
):
    ...
    def test_correct_chunk_sizes(self):
        self.assert_expected_chunk_sizes(
            self.num_ways * self.num_support, self.num_ways * self.num_query
        )
...
class NotEnoughExamplesSamplerTest(EpisodeDescrSamplerTest):
    ...
    def test_skip_classes(self):
        ...
        for _ in range(10):
            ...
            self.assertAllInSet(
                [cid for cid, _, _ in episode_description], expected_classes
            )
    ...
    def test_noskip_at_min(self):
        ...
        for _ in range(10):
            ...
            if any(cid % 3 == 0 for cid, _, _ in episode_description):
                ...
                raise AssertionError(
                    "Classes with exactly `min_examples_in_class` " "were not sampled."
                )
...
class ChunkSizesTest(tf.test.TestCase):
    ...
    def test_large_ways(self):
        ...
        self.assertGreater(support_chunk_size, test_utils.MAX_SUPPORT_SET_SIZE)
        self.assertGreater(
            query_chunk_size, test_utils.MAX_WAYS_UPPER_BOUND * test_utils.MAX_NUM_QUERY
        )
        sampler.sample_episode_description()
