...
def log_data_augmentation(data_augmentation, name):
    ...
    if not data_augmentation:
        ...
        logging.info(
            "enable_gaussian_noise: %s", data_augmentation.enable_gaussian_noise
        )
...
def flush_and_chunk_episode(example_strings, class_ids, chunk_sizes):
    ...
    """
    example_strings_chunks = tf.split(example_strings, num_or_size_splits=chunk_sizes)[
        1:
    ]
...
def process_dumped_episode(
    support_strings, query_strings, image_size, support_decoder, query_decoder
):
...
def add_simclr_episodes(simclr_episode_fraction, *episode):
    ...
    def convert_to_simclr_episode(
        ...
        query_idx_within_class = tf.cast(
            tf.equal(query_labels[tf.newaxis, :], query_labels[:, tf.newaxis]), tf.int32
        )
    ...
    return tf.cond(
        ...
        lambda: episode,
        lambda: convert_to_simclr_episode(*episode),
    )
...
def simclr_augment(image_batch, blur=False):
    ...
    image_batch = tf.map_fn(
        lambda x: data_util.preprocess_for_train(x, h, w, impl="simclrv1"), image_batch
    )
    if blur:
        image_batch = tf.map_fn(lambda x: data_util.random_blur(x, h, w), image_batch)
...
def process_episode(
    ...
    if query_decoder:
        query_images = tf.map_fn(
            query_decoder, query_strings, dtype=query_decoder.out_type, back_prop=False
        )
...
def make_multisource_episode_pipeline(
    dataset_spec_list,
    ...
    if num_to_take is not None and len(num_to_take) != len(dataset_spec_list):
        raise ValueError(
            "num_to_take does not have the same length as " "dataset_spec_list."
        )
    if num_to_take is None:
