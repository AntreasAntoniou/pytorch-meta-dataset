...
def episode_representation_generator(dataset_spec, split, pool, sampler):
    ...
    * some number of additional chunks (for example, a "support" chunk and a
    ...
    total_images_per_class = dict(
        (class_idx, dataset_spec.get_total_images_per_class(class_set[class_idx]))
    ...
    while True:
        ...
        for element in episode_description:
            ...
            if total_requested > total_images_per_class[class_idx]:
                raise ValueError(
                    "Requesting more images than what's available for the "
                    "whole class"
                )
            ...
            for num_to_allocate, dataset_indices in zip(
                distribution, selected_dataset_indices
            ):
        ...
        for dataset_indices, chunk_size in zip(
            selected_dataset_indices, other_chunk_sizes
        ):
            _pad(dataset_indices, chunk_size, placeholder_dataset_id)

        episode_representation = np.array(
            list(itertools.chain(flushed_dataset_indices, *selected_dataset_indices)),
            dtype="int64",
        )
...
class Reader(object):
    ...
    def construct_class_datasets(
        self, pool=None, repeat=True, shuffle=True, shuffle_seed=None
    ):
        ...
        for dataset_id in range(self.num_classes):
            ...
            if pool:
                ...
                if file_pattern.startswith("{}_{}"):
                    ...
                    raise NotImplementedError(
                        "Sharded files are not supported yet. "
                        "The code expects one dataset per class."
                    )
                elif file_pattern.startswith("{}"):
                    filename = os.path.join(
                        self.base_path, file_pattern.format(class_id)
                    )
                else:
                    raise ValueError(
                        "Unsupported file_pattern in DatasetSpec: %s. "
...
class EpisodeReaderMixin(object):
    ...
    def create_dataset_input_pipeline(self, sampler, pool=None, shuffle_seed=None):
        ...
        if not self.num_unique_descriptions:
            choice_dataset = tf.data.Dataset.from_generator(
                representation_generator, (tf.int64), tf.TensorShape([None, 2])
            )
        else:
            ...
            representations = list(
                map(
                    ...
                    itertools.islice(
                        representation_generator(), self.num_unique_descriptions
                    ),
                )
            )
            choice_dataset = (
                tf.data.Dataset.from_tensor_slices(representations)
                .shuffle(self.num_unique_descriptions)
                .map(lambda s: tf.io.parse_tensor(s, tf.int64))
                .repeat()
            )

        choice_dataset = choice_dataset.map(decompress_episode_representation).unbatch()

        dataset = tf.data.experimental.choose_from_datasets(
            class_datasets, choice_dataset
        )
...
class BatchReaderMixin(object):
    ...
    def create_dataset_input_pipeline(
        self, batch_size, offset=0, pool=None, shuffle_seed=None
    ):
        """Creates a Dataset encapsulating the input pipeline for one data source.
        ...
        if pool and pool not in ["valid", "test"]:
            raise ValueError(
                "Invalid pool provided. The supported values " 'are "valid" and "test".'
            )
        ...
        if pool:
            ...
            if list(class_set) != list(range(num_classes)):
                raise NotImplementedError(
                    "Batch training currently assumes the class "
