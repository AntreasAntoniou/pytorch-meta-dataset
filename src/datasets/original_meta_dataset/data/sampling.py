...
def sample_support_set_size(
    num_remaining_per_class, max_support_size_contrib_per_class, max_support_set_size
):
...
def sample_num_support_per_class(
    ...
    * The number of support examples per class is no greater than the number of
    ...
    support_set_proportions = unnormalized_proportions / unnormalized_proportions.sum()

    # This guarantees that there is at least one support example per class.
    num_desired_per_class = (
        np.floor(support_set_proportions * remaining_support_set_size).astype("int32")
        + 1
    )
...
class EpisodeDescriptionSampler(object):
    ...
    def __init__(
        ...
        if self.use_bilevel_hierarchy:
            ...
            if not isinstance(
                dataset_spec, dataset_spec_lib.BiLevelDatasetSpecification
            ):
                raise ValueError(
    ...
    def sample_class_ids(self):
        ...
        if self.use_dag_hierarchy and RNG.choice([True, False], p=prob):
            ...
            episode_subclass_ids = sample_class_ids_uniformly(
                num_ways, num_superclass_classes
            )
            (
                episode_classes_rel,
                _,
        ...
        if self.num_support is not None:
            if isinstance(self.num_support, int):
                ...
                if any(end + num_query > images_per_class):
                    raise ValueError(
                        "The range provided for uniform sampling of the "
