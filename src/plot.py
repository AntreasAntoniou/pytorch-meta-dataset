...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--linewidth", type=int, default=2.0)
    ...
    parser.add_argument(
        "--dpi", type=int, default=200, help="Dots per inch when saving the fig"
    )
    parser.add_argument(
        "--max_col", type=int, default=1, help="Maximum number of columns for legend"
    )

    parser.add_argument("--reduce_by", type=str, default="seed")
    parser.add_argument(
        "--simu_params", type=str, nargs="+", default=["method", "base_source", "arch"]
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    for path in all_files:
        ...
        filenames_dic[metric][fixed_key][reduce_key]["num_updates"] = config[
            "num_updates"
        ]
        filenames_dic[metric][fixed_key][reduce_key]["log_freq"] = (
            config["print_freq"] if "train" in metric else config["eval_freq"]
        )
    ...
    for metric in filenames_dic:
        ...
        ax = fig.gca()
        for style, color, simu_args in zip(
            cycle(styles), cycle(colors), filenames_dic[metric]
        ):
            values = []
            ...
            ax.fill_between(
                x[valid_iter],
                mean[valid_iter] - std[valid_iter],
                mean[valid_iter] + std[valid_iter],
                color=color,
                alpha=0.3,
            )
