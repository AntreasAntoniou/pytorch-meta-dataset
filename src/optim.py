...
def get_scheduler(
    args: Namespace, optimizer: torch.optim.Optimizer
    ...
    SCHEDULER = {
        ...
        "multi_step": MultiStepLR(
            optimizer,
            milestones=[int(0.5 * args.num_updates), int(0.75 * args.num_updates)],
            gamma=args.gamma,
        ),
...
def get_optimizer(args: Namespace, model: torch.nn.Module) -> torch.optim.Optimizer:
    OPTIMIZER = {
        "SGD": torch.optim.SGD(
            model.parameters(),
            lr=args.lr,
