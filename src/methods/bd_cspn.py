...
class BDCSPN(FewShotMethod):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        if args.val_batch_size > 1:
            raise ValueError(
                "For now, only val_batch_size=1 is support for LaplacianShot"
            )
        super().__init__(args)
    ...
    def rectify(self, support: Tensor, query: Tensor, y_s: Tensor, y_q: Tensor) -> None:
        ...
        normalization = (
            (one_hot_s * logits_s).sum(1) + (one_hot_q * logits_q).sum(1)
        ).unsqueeze(1)
    ...
    def forward(
        ...
        self.rectify(support=feat_s, y_s=y_s, query=feat_q, y_q=y_q)
